...
def read(path, template):
    ...
    # The score box holds at most two digits. A 3+ digit read (e.g. "744") or a
    # negative is an OCR artifact, not a real score — drop it so it can't poison
    # the consensus or freeze a "crazy" final.
    if away_score is not None and not (0 <= away_score <= 99):
        away_score = None
    if home_score is not None and not (0 <= home_score <= 99):
        home_score = None
